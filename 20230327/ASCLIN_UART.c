...
uint8 g_rxData[SIZE] = {''};
uint8 protocal_test[10] = { 0, };
uint8 parsing_RX_BUFF[7] = { 0, };
uint8 send_receive_buff[7] = { 0, };
...
void RX_BUFF(void)
{
    if((parsing_RX_BUFF[0]==0xb8) && (parsing_RX_BUFF[1]==0xae))
    {
        if(parsing_RX_BUFF[2]== 'O')
        {
            crc_16_val.check_sum = CRC16_MODBUS(parsing_RX_BUFF, 5);

            if((crc_16_val.b[1] == parsing_RX_BUFF[6]) && (crc_16_val.b[0] == parsing_RX_BUFF[5]))
            {
                data_num = parsing_RX_BUFF[3];
                O_data = parsing_RX_BUFF[4];
            }
        }
    }
}
...
{
    count10++;
    IfxAsclin_Asc_read(&g_ascHandle, parsing_RX_BUFF , &r_count, TIME_INFINITE);  /* Transmit data via TX */
    return parsing_RX_BUFF[0];
}
...
void data_in_4(int data, int adc_v){
    a=data;
    adc_val.adc_valu=adc_v;
}
